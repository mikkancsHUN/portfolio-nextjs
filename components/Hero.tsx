    ...
    >
      ...
      >
        <motion.div variants={itemVariants} className="flex items-center gap-2 mb-6">
          ...
          <span className="text-[10px] uppercase tracking-[0.2em] text-white/50">Available for new projects</span>
        </motion.div>

        <motion.span variants={itemVariants} className="text-sm tracking-[0.3em] uppercase text-[#d326d3] font-light">
          Frontend Developer
        </motion.span>
        
        <motion.h1 variants={itemVariants} className="text-4xl sm:text-6xl lg:text-[5rem] font-medium text-white leading-tight mt-6 max-w-[650px]">
          Building <span className="text-white/80 font-light">digital</span> <br /> 
          <span className="text-[#d326d3] font-bold">experiences</span>.
        </motion.h1>
        
        <motion.p variants={itemVariants} className="mt-6 mb-10 text-neutral-400 font-light leading-relaxed text-sm sm:text-base max-w-[450px]">
          Modern, prémium felhasználói felületeket építek React, TypeScript és Tailwind technológiákkal. 
          Célom a tiszta kód, a tökéletes animációk és olyan digitális termékek létrehozása.
        </motion.p>

        {/* Stats Sor */}
        <motion.div variants={itemVariants} className="flex gap-8 mb-10 border-l border-[#d326d3]/30 pl-6">
          <div>
            <div className="text-2xl font-bold text-white">3+</div>
            <div className="text-[9px] uppercase tracking-widest text-neutral-500">Years Exp.</div>
          </div>
          <div>
            <div className="text-2xl font-bold text-white">15+</div>
            <div className="text-[9px] uppercase tracking-widest text-neutral-500">Projects</div>
          </div>
        </motion.div>
        
<motion.div variants={itemVariants} className="flex flex-wrap gap-6 mt-4">
  
  {/* Elsődleges gomb: Offset keretes animációval */}
  <a 
    href="#projects" 
  ...
  {/* Másodlagos gomb: Letisztult, vékony keretes */}
  <a 
    href="/cv.pdf" 
    target="_blank"
