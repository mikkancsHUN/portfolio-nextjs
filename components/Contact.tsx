          ...
          >
            {['name', 'email', 'message'].map((field) => (
              <div key={field} className="flex flex-col gap-2">
                <label className="text-[10px] text-white/50 uppercase tracking-widest">{field}</label>
                {field === 'message' ? (
                  <textarea rows={4} className="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm text-white focus:outline-none focus:border-[#d326d3] transition-all resize-none" />
                ) : (
                  <input type={field === 'email' ? 'email' : 'text'} className="w-full bg-white/5 border border-white/10 px-4 py-3 text-sm text-white focus:outline-none focus:border-[#d326d3] transition-all" />
                )}
              </div>
            ))}
